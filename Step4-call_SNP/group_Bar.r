...
# create df
df <- data.frame(x=data[,opt$x.col], group=as.factor(data[,opt$group.col]), y=data[,opt$y.col])
...
unique(df$group)
print(col)
col2=c("#FBC29C","#A5DDC2","#4DAF4A")
p <- ggplot(df,aes(x=x, y=y,fill=group))+geom_col(position = position_dodge(width = 0.7),width = 0.5)+scale_fill_manual(values = col2)
##, position="dodge", fill=group)
if (! is.null(opt$max) ){
	p <-p +ylim(0,opt$max)+scale_y_continuous(breaks=c(0,opt$max/2,opt$max))
} 
...
p <- p + guides(fill = guide_legend(ncol = opt$legend.col))+coord_flip()
# grid and background
