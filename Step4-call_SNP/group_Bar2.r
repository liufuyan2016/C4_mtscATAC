...
2) Rscript dodgedBar.r --infile in_dodgedBar.data --outfile out_dodgedBar.png \\
	--x.col 1 --group.col 2 --y.col 3 --x.lab \"x lab\" --group.lab \"group lab\" \\
	--y.lab \"y lab\" --title.lab \"title lab\" --height 3000 --width 4000
...
# create df
df <- data.frame(x=data[,opt$x.col], group=data[,opt$group.col], y=data[,opt$y.col])
df$x=factor(df$x,level=unique(df$x))

df$group=factor(df$group,level=unique(df$group))
...
if(length(unique(df$group))>9){
	col=brewer.pal(n = length(unique(df$group)), name = "Paired")
}else{
	col=brewer.pal(n = length(unique(df$group)), name = "Set1")
}
p <- ggplot(df,aes(x=x, y=y,fill=group))+geom_col(position = position_dodge(width = 0.7),width = 0.5)+scale_fill_manual(values = col)
...
p <- p + theme(title = element_text(face="bold", size=opt$lab.size), 
	axis.text = element_text(face="bold", size=opt$axis.size,angle=45,hjust=1,vjust=1))
