...
if ( is.null(opt$lab.size ) )		{ opt$lab.size = 8 }
if ( is.null(opt$axis.size ) )		{ opt$axis.size = 8 }
...
#-----------------------------------------------------------------
# mian plot
col=brewer.pal(n = length(unique(df$x)), name = "Set3")
print(class(col))
p <- ggplot(df, aes(x=x,y=y))+ geom_violin(aes(fill =x), trim = FALSE) +theme_minimal() +scale_fill_manual(values = col)#+geom_signif(comparisons = comparisons, map_signif_level = T, textsize = 1, test = wilcox.test, step_increase = 0.05) 
# density
